 ...
 */
export async function runImapFetch(limit = 10): Promise<ImapResult> {
  const env = loadEnv();
  ...
  const createClient = (host: string, port: number, secure: boolean) =>
    new ImapFlow({
      host,
      port,
      secure,
  ...
  try {
    try {
      ...
      notes.push(
        `connect failed (host=${IMAP_HOST} port=${IMAP_PORT} secure=${!!IMAP_TLS}) name=${err.name || ''} message=${err.message}`
      );
      ...
      try {
        ...
        const err2 = e2 as Error & { code?: string };
        notes.push(
          `fallback connect failed (host=${IMAP_HOST} port=143 secure=false) name=${err2.name || ''} message=${err2.message}`
        );
        throw e2; // give up
      }
    }
    ...
    const maxUid = Number((status as any).uidNext || 1) - 1;
    ...
    try {
      for await (const msg of client.fetch(range, { envelope: true, internalDate: true, bodyStructure: true, uid: true })) {
        ...
        try {
          ...
          const messageId = normalizeMessageId((msg.envelope as any)?.messageId);
          ...
          const attachmentsMeta: { filename: string; mime: string; size: number }[] = [];
          let parsed = false;
          let parseError: string | undefined;
          let rawEmailId: string | undefined;

          const parts: MessageStructureObject[] = flattenParts(msg.bodyStructure as any);
          const attachParts = parts.filter(p => !!p.disposition && (p.disposition as any).type?.toLowerCase() === 'attachment');
          ...
          for (const part of attachParts) {
            const filename = (part.disposition as any)?.params?.filename || (part as any).parameters?.name || 'attachment';
            const subtype = (part as any).subtype || 'octet-stream';
          ...
          errors += 1;
          notes.push(`save error: ${(e as Error).message}`);
        }
      }
    } finally {
      lock.release();
    ...
    errors += 1;
    notes.push(`imap error: ${(e as Error).message}`);
  } finally {
    try { await client.logout(); } catch {}
    try { await client.close(); } catch {}
  }
 ...
 */
export async function runImapBackfill(limit = 50): Promise<ImapResult> {
  const env = loadEnv();
  const { IMAP_HOST, IMAP_PORT, IMAP_USER, IMAP_PASS, IMAP_TLS } = env;
  ...
  try {
    ...
    try {
      for (const r of raws) {
        if (!r.imapUid) continue;
        fetched += 1;
        try {
          const uid = r.imapUid;
          // fetch single message structure by UID (use fetchOne to avoid open iterators)
          const msg = await client.fetchOne(uid, { bodyStructure: true, uid: true }).catch(() => undefined as any);
          if (!msg) { notes.push(`uid ${uid} not found`); continue; }
          ...
          for (const part of attachParts) {
            ...
            const { content } = await client.download(uid, (part as any).part);
            const buf = await streamToBuffer(content);
            try {
              if (isXML) {
                const xml = buf.toString('utf8');
          ...
          errors += 1;
          notes.push(`backfill save error: ${(e as Error).message}`);
        }
      }
    } finally {
      lock.release();
...
function flattenParts(node: MessageStructureObject | MessageStructureObject[] | undefined): MessageStructureObject[] {
  if (!node) return [];
  const arr = Array.isArray(node) ? node : [node];
  const out: MessageStructureObject[] = [];
  for (const n of arr) {
    out.push(n);
    if ((n.childNodes as any)?.length) {
      out.push(...flattenParts(n.childNodes as any));
    }
  }
  return out;
}
...
async function extractZipXMLs(buf: Buffer): Promise<string[]> {
  const xmls: string[] = [];
...
async function parseAndPersist(xml: string, rawEmailId?: string) {
 ...
 */
export async function downloadAttachmentByFilename(
  imapUid: number,
  filename: string
): Promise<{ buffer: Buffer; mime: string; dispositionFilename: string }> {
  const env = loadEnv();
  const { IMAP_HOST, IMAP_PORT, IMAP_USER, IMAP_PASS, IMAP_TLS } = env;
  if (!IMAP_HOST || !IMAP_PORT || !IMAP_USER || !IMAP_PASS) {
  ...
  const createClient = (host: string, port: number, secure: boolean) =>
    new ImapFlow({ host, port, secure, auth: { user: IMAP_USER, pass: IMAP_PASS }, socketTimeout: 60_000, logger: console });
  ...
  try {
    ...
    const msg = await client.fetchOne(imapUid, { bodyStructure: true, uid: true }).catch(() => undefined as any);
    if (!msg) throw new Error('message not found');
    const parts = flattenParts(msg.bodyStructure as any);
    const lower = filename.toLowerCase();
    const attach = parts.find((p) => {
      const disp = (p as any).disposition;
      const name = disp?.params?.filename || (p as any).parameters?.name;
      if (!disp || (disp.type || '').toLowerCase() !== 'attachment') return false;
      return String(name || '').toLowerCase() === lower;
    });
    if (!attach) throw new Error('attachment not found');

    const subtype = (attach as any).subtype || 'octet-stream';
    const mime = `${(attach as any).type}/${subtype}`.toLowerCase();
    const { content } = await client.download(imapUid, (attach as any).part);
    const buffer = await streamToBuffer(content);
    const dispName = (attach as any).disposition?.params?.filename || filename;
