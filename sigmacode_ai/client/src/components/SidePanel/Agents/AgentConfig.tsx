...
export default function AgentConfig({ createMutation }: Pick<AgentPanelProps, 'createMutation'>) {
  ...
  const handleCreateSimDraft = useCallback(async () => {
    try {
      setSimCreating(true);
      const payload = {
        name: agentName ?? 'Untitled Agent Flow',
        description: agentDescription ?? '',
        agent_id,
        model,
        tools: tools ?? [],
        // Optional: zusätzliche Hinweise/Files zur späteren Abbildung im Flow
        knowledge_files,
        context_files,
        code_files,
      };

      const resp = await fetch('/api/sim/flows/create', {
        method: 'POST',
      ...
      if (!resp.ok) {
        ...
        throw new Error(err?.error || `Sim returned ${resp.status}`);
      }

      const data = await resp.json();
      showToast({
        status: 'success',
        message: `Sim-Entwurf angelegt${data?.id ? ` (ID: ${data.id})` : ''}`,
      });
    } catch (e: any) {
      showToast({
        status: 'warning',
        message: `Sim-Entwurf fehlgeschlagen: ${e?.message || e}`,
      });
    } finally {
      setSimCreating(false);
    }
  }, [agentName, agentDescription, agent_id, model, tools, knowledge_files, context_files, code_files, showToast]);
  ...
  return (
    <>
      <div className="h-auto bg-white px-4 pt-3 dark:bg-transparent">
        ...
        {/* SIGMACODE AI Workflow Draft */}
        <div className="mb-4">
          <label className={labelClass}>
            SIGMACODE AI Workflow
          </label>
          <button
            type="button"
            onClick={handleCreateSimDraft}
            disabled={simCreating}
            className="btn btn-neutral border-token-border-light relative h-9 w-full rounded-lg font-medium"
          >
            <div className="flex w-full items-center justify-center gap-2">
              {simCreating ? localize('com_ui_loading') : 'Als SIGMACODE AI Workflow-Entwurf anlegen'}
            </div>
          </button>
        </div>
        {/* Description */}
