...
import { Workflow, Layers3, FileText, ShieldCheck, LineChart } from 'lucide-react';
...
export default function HowToOrchestrate() {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 dark:bg-gray-950 dark:text-gray-50">
      <SEO
        title={`${tt('marketing.howto.orchestrate.title')} · SIGMACODE AI`}
        description={tt('marketing.howto.orchestrate.description')}
        canonical="/how-it-works/orchestrate"
        keywords={[tt('marketing.howto.orchestrate.badge'), ...items.map(i => i.title)]}
        jsonLd={[
          {
            ...
            itemListElement: [
              ...
              { '@type': 'ListItem', position: 2, name: tt('marketing.howto.orchestrate.badge'), item: '/how-it-works/orchestrate' },
            ],
          },
          {
            '@context': 'https://schema.org',
            '@type': 'TechArticle',
            headline: tt('marketing.howto.orchestrate.title'),
            description: tt('marketing.howto.orchestrate.description'),
            mainEntityOfPage: '/how-it-works/orchestrate',
          },
        ]}
      />
      <main className="mx-auto max-w-7xl px-4 py-14 md:py-20">
        <nav className="mb-6 text-sm text-gray-600 dark:text-gray-400" aria-label="Breadcrumb">
          ...
          <span>{tt('marketing.howto.orchestrate.badge')}</span>
        </nav>

        <SectionBadge icon={Workflow} variant="glass">{tt('marketing.howto.orchestrate.badge')}</SectionBadge>
        <h1 className="mt-2 text-3xl font-bold">{tt('marketing.howto.orchestrate.title')}</h1>
        <p className="mt-2 max-w-3xl text-gray-700 dark:text-gray-300">{tt('marketing.howto.orchestrate.description')}</p>

        <h2 className="sr-only">{tt('marketing.howto.orchestrate.title')}</h2>
        <NumberedSteps
          steps={items}
          colsMd={2}
          prefixKeyOverride="marketing.howto.orchestrate.steps.prefix"
          ariaKeyOverride="marketing.howto.orchestrate.steps.aria"
        />

        <div className="mt-10 text-base md:text-[17px] leading-relaxed text-gray-700 dark:text-gray-300">
          <h3 className="font-semibold">{tt('marketing.howto.orchestrate.badge')}</h3>
        ...
        {/* Unified CTA Panel */}
        <section className="mt-12" aria-labelledby="howto-orchestrate-cta-title">
          <h2 id="howto-orchestrate-cta-title" className="sr-only">{tt('marketing.howto.cta.title')}</h2>
